...
Next, we will introduce a few packages
...
"edges" when working with network data and the range of ties in a
...
and then `select()` those columns along with any attributes that we
think might be useful for analysis later on, like the timestamp or
content of the tweet, i.e. `text`.
...
that `unnest_tokens` expects a data frame as the first argument, which
...
-   By default, words are changed to lowercase, which makes them easier
    ...
    the `to_lower = FALSE` argument to turn off this behavior since
