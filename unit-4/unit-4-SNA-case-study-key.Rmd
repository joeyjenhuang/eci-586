...
Lab^TM^*. Similar to an approach we'll used in Unit 3, the authors
...
example, take a look at the [*Explore the
...
italicized) and copy their definition used. Note, Transenders has been
completed for you.
...
First, load the {tidyverse} and {tidytext} packages since we'll be using
several of these packages to wrangling and explore our data later
...
things simple. Use the `read_csv()` function from the {readr} package to
read the `ccss-tweets-fresh.csv` file from Unit 2 and saved in our data
folder:
...
If you'd like to learn more about the process for how these tweets were
retrieved, visit the short tutorial on using [rtweet & the Twitter
API](https://laser-institute.github.io/text-mining/keys/tm-learning-lab-4-key.html#1_PREPARE).
...
b.  **Create a Network Object**. Before performing network analyses,
    we'll need to convert our data frames into special data format, an R
    network object, for working with relational data.

c.  **Simplify Network**. Finally, we'll pare down our network and learn
    about a handy `simplify()` function in the {igraph} package for
    collapsing multiple ties between actors and removing "self-loops."
...
have likely worked with before in that the values in the first two
columns of each row represent a dyad, or tie between two nodes in a
network. An edge-list can also contain other information regarding the
strength, duration, or frequency of the relationship, sometime called
weight, in addition to other "edge attributes."
...
Use the code below to look at the data we just imported using one of
your favorite methods for inspecting data. In the space below, identify
the columns you think could be used to construct an edge list.
...
As noted above, an edgelist include the nodes that make up a tie or
...
included in the mentions, let's `relocate()` and rename those columns,
then `select()` those columns along with any attributes that we think
might be useful for analysis later on, like the timestamp or content of
the tweet, i.e. `text`.
...
Before we used the `unnest_tokens()` function to extract individual
words from tweets. This time we'll use it to place each username
mentioned into separate rows corresponding to the user that mentioned or
replied too these users:
...
Use the code chunk below to take a quick look at our final edgelist and
...
How many edges are in our CCSS network and what do these
ties/edges/connections represent?
...
To construct our basic nodelist, we will use tthe `pivot_longer()`
...
Run the following code to select the usernames form our `ties` data
frame, merge our sender and receiver columns into a single column, and
take a quick look at our new data frame.
...
How many nodes are in our CCSS network?
...
-   `directed =` specifies whether the constructed graph should be
    ...
    sender or target of a connection.
...
undirected, however, since somene can not "friend" someone on Facebook
without them also indicating a "friend" relationship.
...
measures of centrality. Complete the code chunk below to add an
out-degree measure to each node and print the output:
...
Now create a new `node_measures` data frame like with did above using
the `ccss_network` data frame you just made, then create a `summary()`
of your node measures like above, and print the output of
`node_measures`:
...
following types of central actors identified:
...
professional development workshop, this might have been a little more
useful, but for larger networks like our Twitter data, this doesn't
...
As you can see, this didn't produce much. Just like our corresponding
`ggplot()` function, all that `ggraph()` does is set up the network
object to make a sociogram, and create a layout, in this case the
default "stress" layout, for our network.

#### Add Nodes

Very similar to how `ggplot()` use the `+` operator to "layer" functions
together to progressively build graphs, functions in the tidyverse use
the `%>%` operator to "pipe" functions together and progressively
wrangle data, `ggraph` use the `+` operator progressively build a
sociogram.
...
for our sociogram is not too great. Let's fix that.
...
That's a little better. We can start to make our our separate network
"components" as distinct groups, with one larger connected group in the
center.

#### Tweak Nodes

Also like {ggplot2}, geoms can include aesthetics, or aes for short,
such as `alpha` for transparency, as well as `colour`, `shape` and
`size`.

Let's now add some "aesthetics" to our points by including the `aes()`
function and arguments such as `size =` and color, which we can set to
our `in_degree` measures:
...
highlight some of the main "transmitters" in our network.

#### Add Edges

Now, let's connect the dots and add some edges using the
`geom_edge_link()` function. We'll also include some arguments like
`arrow =` to include some arrows 1mm in length, and `end_cap =` to keep
arrows from overlapping the nodes, and finally set the transparency of
our edges to `alpha = .2` so they fade more into the background and help
keep the focus on our nodes:
...
Let's add `theme_graph()` to our sociogram, remove the legends, and call
it good for now:
...
Now that we've assigned our nodes to a group, let's add one more layer
...
introduce in Unit 3 to help answer this question. Let's give VADER
`ties` data frame that we created earlier and include the `$` operator
to select for analysis our `text` column containing our tweets.
...
Now that we have these joined, we can take a quick look at the
sentiment.
