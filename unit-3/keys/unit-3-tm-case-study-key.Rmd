...
### 2a. Part 1: Import Tweets from CSV
...
Use the following code chunk to import the NGSS tweets located in the same data folder as our common core tweets. By default, R will treat numerical IDs in our dataset as numeric values but we will need to convert these to characters for the purposes of analysis since these unique identifiers:
...
**RStudio Tip:** Importing data and dealing with data types can be a bit tricky, especially for beginners. Fortunately, RStudio has an "Import Dataset" feature in the Environment Pane that can help you use the {readr} package and associated functions to greatly facilitate this process. If you get stuck, you can copy the code generated from

![](images/import-data.png){width="80%"}

Now use the following code chunk to inspect your data and answer the questions that follow:
...
1.  What do you notice about our data sets? Record a few observations that you think are relevant to our analysis or might be useful for future analyses.

    -   

2.  Based on a quick scan of the `text` column we'll be analyzing, what do these tweets seem to be about and which set of tweets do you think will expresses a more negative sentiment?
...
### 2a. Part 2: Import Tweets from API (Optional)

This section is optional and for those those [setup a Twitter Developer Account and App](https://laser-institute.github.io/laser-sundry/twitter-setup.html#Introduction). The Import Tweets section introduces the following functions from the {rtweet} package for reading Twitter data into R:
...
`{r} ngss_all_tweets <- search_tweets(q = "#NGSSchat", n=5000)}`
...
View your new `ngss_all_tweets`data frame using one of the previous view methods from Unit 1 Section 2a to help answer the following questions:
...
`{r} ngss_non_retweets <- search_tweets("#NGSSchat", n=5000, include_rts = FALSE)}`
...
`{r} ngss_or_tweets <- search_tweets(q = "#NGSSchat OR ngss", n=5000, include_rts = FALSE)}`
...
`{r} ngss_tweets <- search_tweets2(c("#NGSSchat OR ngss", '"next generation science standard"', '"next generation science standards"', '"next gen science standard"', '"next gen science standards"' ), n=5000, include_rts = FALSE)}`
...
`{r} ngss_dictionary <- c("#NGSSchat OR ngss", '"next generation science standard"', '"next generation science standards"', '"next gen science standard"', '"next gen science standards"') ngss_tweets <- search_tweets2(ngss_dictionary, n=5000, include_rts = FALSE)}`
...
`{r} ccss_dictionary <- c("#commoncore", '"common core"') ccss_tweets <- ccss_dictionary %>% search_tweets2(n=5000, include_rts = FALSE)}`
...
`{r} fi <- c("sbkellogg", "mjsamberg", "haspires", "tarheel93", "drcallie_tweets", "AlexDreier") fi_tweets <- fi %>% get_timelines(include_rts=FALSE)}`
...
`{r} sample_n(fi_tweets, 10) %>% select(screen_name, text)}`
...
```{r, eval=F} vignette("intro", package="rtweet")}
```
...
write_csv(ss_tokens_3, here("data", "ss_tidy_tweets.csv"))
```
