...
4.  **Model:** We learn about sentiment lexicons and introduce the {vader} package to model the sentiment of tweets about the NGSS and CCSS state standards in order to better understand public reaction to these two curriculum reform efforts.
...
For our text mining case study, we'll use approaches similar to those used by the authors cited above to better understand public discourse surrounding these standards, particularly as they relate to STEM education. We will also try to gauge public sentiment around the NGSS, by comparing how much more positive or negative NGSS tweets are relative to CSSS tweets. Specifically, in this case study we'll attempt to answer the following questions:
...
For those that have Twitter account or created a Twitter Developer account, load the {rtweet} package that we'll be using to accomplish all three of the goals listed above:
...
```{r, eval=FALSE}
fi <- c("sbkellogg", "tarheel93", "drcallie_tweets") 
...
1.  How do our results compare to the CSSS sample of tweets?

    -   YOUR RESPONSE HERE

2.  How do these result compare to those found by Rosenberg et al. (2021)?
...
1.  One important thing I took away from this case study about text mining :

    -   YOUR RESPONSE HERE

2.  One thing about text mining I want to learn more about:
